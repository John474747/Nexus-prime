...
export function ProductDiscovery() {
  ...
  const handleProduct = (prod: typeof PRODUCTS[0]) => {
    if (prod.route) {
      navigate(prod.route)
    } else {
      toast({
        title: `${prod.title} — Coming Soon`,
        description: `Launching ${prod.eta ?? 'soon'}. We'll notify you when it's ready.`,
      })
    }
  }
        ...
        <div className="flex w-max space-x-5 px-5 pb-6">
          {PRODUCTS.map((prod) => (
            <div
              ...
              className={cn(
                'w-[160px] p-6 rounded-[32px] flex flex-col shrink-0 relative h-[200px] shadow-soft border border-white/50 cursor-pointer active:scale-95 transition-transform',
                prod.color,
              )}
            >
              ...
              <button
                className={cn(
                  'absolute bottom-5 right-6 w-9 h-9 rounded-full flex items-center justify-center text-white shadow-lg active:scale-90 transition-all',
                  prod.btnColor,
                )}
              >
                <ArrowRight size={18} />
              ...
              {!prod.route && prod.eta && (
                <span className="absolute top-4 right-4 text-[8px] font-bold text-gray-400 uppercase tracking-widest">
                  {prod.eta}
                </span>
              )}
            </div>
